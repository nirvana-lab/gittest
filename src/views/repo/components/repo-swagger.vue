    ...
    SwaggerUI({
      ...
      spec: {
        ...
        paths: {
          ...
          '/pet/{petId}': {
            get: {
              ...
              responses: { 200: { description: 'successful operation', schema: { $ref: '#/definitions/Pet' } }, 400: { description: 'Invalid ID supplied' }, 404: { description: 'Pet not found' } },
              security: [{ api_key: [] }],
            },
            post: {
              tags: ['pet'],
          ...
          '/store/order/{orderId}': {
            get: {
              ...
              responses: { 200: { description: 'successful operation', schema: { $ref: '#/definitions/Order' } }, 400: { description: 'Invalid ID supplied' }, 404: { description: 'Order not found' } },
            },
            delete: {
              tags: ['store'],
          ...
          '/store/inventory': {
            get: {
              tags: ['store'], summary: 'Returns pet inventories by status', description: 'Returns a map of status codes to quantities', operationId: 'getInventory', produces: ['application/json'], parameters: [], responses: { 200: { description: 'successful operation', schema: { type: 'object', additionalProperties: { type: 'integer', format: 'int32' } } } }, security: [{ api_key: [] }],
            },
          },
          ...
          '/user/{username}': {
            get: {
              ...
              responses: { 200: { description: 'successful operation', schema: { $ref: '#/definitions/User' } }, 400: { description: 'Invalid username supplied' }, 404: { description: 'User not found' } },
            },
            put: {
              tags: ['user'],
          ...
          '/user/login': {
            get: {
              ...
              responses: { 200: { description: 'successful operation', headers: { 'X-Expires-After': { type: 'string', format: 'date-time', description: 'date in UTC when token expires' }, 'X-Rate-Limit': { type: 'integer', format: 'int32', description: 'calls per hour allowed by the user' } }, schema: { type: 'string' } }, 400: { description: 'Invalid username/password supplied' } },
            },
          },
          '/user/logout': {
            get: {
              tags: ['user'], summary: 'Logs out current logged in user session', description: '', operationId: 'logoutUser', produces: ['application/json', 'application/xml'], parameters: [], responses: { default: { description: 'successful operation' } },
            },
          },
          '/user': {
            post: {
        ...
        securityDefinitions: {
          ...
          petstore_auth: {
            type: 'oauth2', authorizationUrl: 'https://petstore.swagger.io/oauth/authorize', flow: 'implicit', scopes: { 'read:pets': 'read your pets', 'write:pets': 'modify pets in your account' },
          },
        },
        definitions: {
          ...
          Pet: {
            ...
            properties: {
              id: { type: 'integer', format: 'int64' }, category: { $ref: '#/definitions/Category' }, name: { type: 'string', example: 'doggie' }, photoUrls: { type: 'array', xml: { wrapped: true }, items: { type: 'string', xml: { name: 'photoUrl' } } }, tags: { type: 'array', xml: { wrapped: true }, items: { xml: { name: 'tag' }, $ref: '#/definitions/Tag' } }, status: { type: 'string', description: 'pet status in the store', enum: ['available', 'pending', 'sold'] },
            },
            xml: { name: 'Pet' },
          },
          ...
          Order: {
            type: 'object',
            properties: {
              id: { type: 'integer', format: 'int64' }, petId: { type: 'integer', format: 'int64' }, quantity: { type: 'integer', format: 'int32' }, shipDate: { type: 'string', format: 'date-time' }, status: { type: 'string', description: 'Order Status', enum: ['placed', 'approved', 'delivered'] }, complete: { type: 'boolean' },
            },
            xml: { name: 'Order' },
          },
          User: {
            type: 'object',
            properties: {
              id: { type: 'integer', format: 'int64' }, username: { type: 'string' }, firstName: { type: 'string' }, lastName: { type: 'string' }, email: { type: 'string' }, password: { type: 'string' }, phone: { type: 'string' }, userStatus: { type: 'integer', format: 'int32', description: 'User Status' },
            },
            xml: { name: 'User' },
          },
        },
